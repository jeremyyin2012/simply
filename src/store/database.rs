...
impl Databases {
    ...
    pub fn doc(&self, collection: String) -> Collection<Value> {
        return self.default.collection::<Value>(collection.as_str());
    }
}
...
pub async fn connect(config: Config) -> mongodb::error::Result<Database> {
    ...
    let mongo_db_name = env::var("MONGO_DB_NAME").unwrap_or("simply".to_string());
